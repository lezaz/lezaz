...
That means that we sometimes have to dynamically generate some CSS that usually gets added to the page's `<head>`. Depending on the number of options we have and the amount of calculations that are required this can be a resources-intensive process, and will not be pretty since we may end up with 100K of CSS inline.

The solution can be to write the dynamically-generated CSS to a file and then load that file instead of re-calculating and enqueuing the CSS on each page load.

This is where this nifty class comes to life.

The implementation here is based on the work I recently did for a commercial theme and though I changed a few things you should be able to make it work without a lot of trouble.
...
class Aristath_Dynamic_CSS {
	...
	public function __construct() {
		...
		$this->stylesheet_id = 'my-theme-style';
...
What this does is generate a separate file for each page. This was done because in that particular use-case a lot of the CSS was depending on options set inside the pages themselves, so each post had to have different CSS and therefore a single CSS file would not do.
However it would be pretty simple to just remove these parts of the code if you don't need them for your own implementation and just keep the parts that make sense to you.

If it fails to add the dynamic CSS in a file then it falls-back to adding the CSS inline in the `<head>` of your document and furthermore there's a limit of 1 file processing every 5 seconds, so even on high-traffic sites this will still work.

To add your dynamic CSS you will have to hook them in the `'dynamic_css'` filter.
